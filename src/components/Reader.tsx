...
import { getWorkDetails, addComment, addRating } from '../data';
...
export default function Reader({ workId, userId, onBack }: { workId: number, userId: number, onBack: () => void }) {
  ...
  useEffect(() => {
    ...
    const res = getWorkDetails(workId);
    if (res) {
      setData(res);
    }
    setLoading(false);
  ...
  const submitRating = async (val: number) => {
    ...
    addRating(workId, userId, val);
  };

  const submitComment = async () => {
    if (!hasFinished || !comment.trim()) return;
    // Use local function instead of API call
    // We need the user's email, which we can get from the user object if we pass it, 
    // or just use a placeholder for now. Let's assume userId is enough for now, 
    // but the data.ts expects an email.
    // Actually, let's just pass a placeholder email if we don't have it.
    addComment(workId, userId, "User", comment);
    setComment('');
    // Refresh data to show new comment
    const newData = getWorkDetails(workId);
    setData(newData);
  };
